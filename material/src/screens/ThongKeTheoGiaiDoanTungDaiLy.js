...
export default function ThongKeTheoGiaiDoanTatCaDaiLy({navigation,route}){
    ...
    const ThongKeDoanhThu = async ()=>{
        await axios.post(
            `${APITKLNGD}`,
            {
                madailyfilter: route.params.daily._id,startyearstatistic : parseInt(yearbd),endyearstatistic: parseInt(yearkt)
            },
                            ...
                            <View style={{display:'flex',justifyContent:'space-around',alignItems:'center',width:400}}>
                                {
                                    ...
                                    dataStatistic.map((item,index) => (
                                            <View key={index} style={{display:'flex',flexDirection:'column',justifyContent:'space-around'}}>
                                                ...
                                                <View style={styles.itemstatistic}>
                                                    <Text>Tổng doanh thu năm {item.nam} : {Format(item.chiphixuat - item.chiphinhap)}</Text>
                                                </View>
                                            </View>
                                ...
                                {
                                    <View style={[styles.itemstatistic,{backgroundColor:'#1b94ff'}]}>
                                        <Text style={{color:'#fff'}}>Doanh thu từ năm {yearbd} - {yearkt} : {Format(onLoadTotal())}</Text>
                                    </View>
                                }
                                 </View>
