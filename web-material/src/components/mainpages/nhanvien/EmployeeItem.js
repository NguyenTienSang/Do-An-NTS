...
function EmployeeItem({ employee, stt, EditEmployee, DeleteEmployee }) {
  ...
  return (
    <div className="employee_item">
      ...
      <div
        style={{ width: "160px" }}
        className="employee_item_element id_employee"
      >
        <p>{employee._id}</p>
      </div>
      ...
      {isAdmin ? (
        <>
          <div style={{ flex: 1 }} className="employee_item_element">
            ...
            >
              <FaUserEdit style={{ color: "rgb(15, 184, 0)" }} />
            </button>
            <button
              style={{ fontSize: 36 }}
              onClick={() =>
                DeleteEmployee(employee._id, employee.images.public_id)
              }
            >
              <RiDeleteBin6Line style={{ color: "red" }} />
            </button>
            <button style={{ fontSize: 36 }}>
              <Link to={`/detail_employee/${employee._id}`}>
