...
function DetailExportBill(exportbill) {
  ...
  return (
    ...
    <div className="title-detail"><p>Chi Tiết Phiếu Xuất {exportbill.exportbill.tenpx}</p></div>
    ...
    }
    {
      exportbill.exportbill.ctpx.map((item,stt) => {
        ...
        <div className="item-detail">
          ...
          <div className="item-detail-element">{Format(item.giaxuat * item.soluong)}</div>
        </div>
        )
  })
    }
