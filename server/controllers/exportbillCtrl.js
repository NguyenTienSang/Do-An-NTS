...
const exportbillCtrl = {
  ...
  createExportBill: async (req, res) => {
    const { ngay, manv, makho, hotenkh, sodienthoaikh, ctpx } = req.body;
    ...
    try {
      ...
      ctpx.map(async (ctpxitem) => {
        const soluongvt = await VatTu.findById(ctpxitem.mavt.toString(), {
          soluong: 1,
          _id: 0,
        });

        await VatTu.findOneAndUpdate(
          { _id: ctpxitem.mavt.toString() },
          { soluong: soluongvt.soluong - ctpxitem.soluong },
          { new: true }
        );
      });
